            ...
            while (true)
            {
                ...
                try
                {
                    foreach (IConfigItem configItem in configuration.GetConfigurationItems())
                    {
                        ...
                        if (configItem.SecondsPassedSinceLastPoll >= Math.Max(1, configItem.AcquisitionInterval))
                        {
                ...
                {
                    string message = string.Format("{0}.{1}: {2}", ex.TargetSite.ReflectedType.Name, ex.TargetSite.Name, ex.Message);
